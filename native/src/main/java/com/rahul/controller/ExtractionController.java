...
public class ExtractionController {
    ...
    private boolean isOnline() {
        try {
            ...
            try {
                java.net.URL url2 = new java.net.URL("https://takeout-fix.firebaseapp.com");
                java.net.URLConnection conn2 = url2.openConnection();
    ...
    public ResponseEntity<?> start(@RequestBody Map<String, String> params) {
        try {
            if (!isOnline()) {
                return ResponseEntity.status(503).body(Map.of("message", "Active internet connection required. TakeoutFix must be online to verify user account status and process archives."));
            }

...
public class UserService {
    ...
    public boolean isAuthenticated() {
        ...
        Map<String, Object> prof = UserController.getCurrentUserProfile();
        if (prof != null && prof.containsKey("email")) {
            String email = (String) prof.get("email");
            if (email != null && !email.trim().isEmpty()) {
                try {
                    String uid = (String) prof.getOrDefault("uid", prof.getOrDefault("googleId", email));
                    String name = (String) prof.getOrDefault("displayName", prof.getOrDefault("name", email));
                    String picture = (String) prof.getOrDefault("photoURL", prof.getOrDefault("picture", ""));
                    loginOrCreateUser(uid, email, name, picture);

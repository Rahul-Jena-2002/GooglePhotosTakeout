...
public class MetadataMatcher {
    ...
    // Matches suffixes like .supplemental-metadata.json, _metadata.json, .supp(1).json, etc.
    // Group 1: Separator (. or _), Group 2: Keyword, Group 3: Optional (N)
    private static final Pattern META_SUFFIX_PATTERN = Pattern.compile("([._])(" + DYNAMIC_REGEX + ")(\\(\\d+\\))?\\.json$", Pattern.CASE_INSENSITIVE);
    ...
    public Set<String> getJsonCandidates(String mediaFilename) {
        ...
        if (lowerExt.equals(".mp4") || lowerExt.equals(".mov") || lowerExt.equals(".m4v") || lowerExt.equals(".3gp")) {
            baseSources.add(nameNoExt + ".jpg");
            baseSources.add(nameNoExt + ".jpeg");
            baseSources.add(nameNoExt + ".heic");
        } else if (lowerExt.equals(".jpg") || lowerExt.equals(".jpeg") || lowerExt.equals(".heic")) {
            baseSources.add(nameNoExt + ".mp4");
            baseSources.add(nameNoExt + ".mov");
        }
        ...
        for (String base : baseSources) {
            if (base.length() > MAX_STEM) {
                ...
                for (String suffix : DYNAMIC_SUFFIXES) {
                    ...
                    // Also support asymmetric sidecar numbering e.g. photo.jpg.supplemental-metadata(1).json
                    stems.add(base + suffix + "(1)");
                    stems.add(base + suffix + "(2)");
                }
            }
        }
        ...
        if (nameNoExt.length() > MAX_STEM) {
            ...
            stems.add(truncate(nameNoExt, MAX_STEM) + "(1)");
            stems.add(truncate(nameNoExt, MAX_STEM) + "(2)");
            stems.add(truncate(nameNoExt, 47));
            stems.add(truncate(nameNoExt, 47) + "(1)");
            stems.add(truncate(nameNoExt, 47) + "(2)");
        } else {
            stems.add(nameNoExt);
        }
    ...
    private Set<String> generateNumberedCandidates(String mediaFilename) {
        ...
        if (m.matches()) {
            ...
            if (base.length() > MAX_STEM) {
                numberedCandidates.add(truncate(base, MAX_STEM) + numberSuffix);
                numberedCandidates.add(truncate(base, 47) + numberSuffix);
            }
            numberedCandidates.add(truncate(base, MAX_STEM));
            numberedCandidates.add(base + ext + numberSuffix);
            numberedCandidates.add(base + numberSuffix + ext);
            numberedCandidates.add(base + ext);
            numberedCandidates.add(base);

            for (String s : DYNAMIC_SUFFIXES) {
                // Suffix with number: base.ext.supplemental-metadata(1)
                numberedCandidates.add(base + ext + s + numberSuffix);
                // Fallback to unnumbered sidecar if multiple duplicates share one JSON
                numberedCandidates.add(base + ext + s);
                numberedCandidates.add(base + s + numberSuffix);
                numberedCandidates.add(base + s);
            }
        }

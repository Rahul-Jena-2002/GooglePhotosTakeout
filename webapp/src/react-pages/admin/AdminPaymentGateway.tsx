...
export default function AdminPaymentGateway() {
  ...
  const [fetchingFromDodo, setFetchingFromDodo] = useState(false)
  const [provisioningAll, setProvisioningAll] = useState(false)
  const [fetchedCatalogData, setFetchedCatalogData] = useState<{
    open: boolean;
    count: number;
    envMode: string;
    products: any[];
  ...
  const handleFetchProductsFromDodo = async () => {
    setFetchingFromDodo(true)
    let cfUrl = ''
    try {
      ...
      if (!dodoKeyToSend) {
        useToastStore.getState().addToast(`Please configure and save your Dodo API Key (${dodoTestMode ? 'Test' : 'Live'}) first.`, 'error')
        setFetchingFromDodo(false)
        return
      }
      ...
      if (data.count === 0) {
        useToastStore.getState().addToast('Connected to Dodo Payments, but no products were found in your account. You can use "Auto-Provision All" to create them.', 'info')
        return
      }

      // Merge newly discovered product IDs into existing dodoProducts state
      ...
      // Save to settings/global in Firestore
      const productField = dodoTestMode ? "dodo_products_test" : "dodo_products_live"
      const updatePayload: any = { [productField]: mergedMap }
      if (!dodoTestMode) updatePayload.dodo_products = mergedMap
      if (data.mappedProductsFull) {
        const fullField = dodoTestMode ? "dodo_products_full_test" : "dodo_products_full"
        updatePayload[fullField] = data.mappedProductsFull
        if (!dodoTestMode) updatePayload.dodo_products_full = data.mappedProductsFull
      }
      await setDoc(doc(db, "settings", "global"), updatePayload, { merge: true })

      // Also update pricing_tiers if mappedPrices was returned
      ...
      setFetchedCatalogData({
        open: true,
        count: data.count || 0,
        envMode: data.envMode || (dodoTestMode ? 'test' : 'live'),
        products: data.products || [],
        mappedProducts: mergedMap,
        mappedPrices: data.mappedPrices || {}
      })

      useToastStore.getState().addToast(`✅ Successfully fetched & synced ${matchedCount} tier products from Dodo (${data.envMode.toUpperCase()} mode)!`, 'success')

    } catch (err: any) {
      useToastStore.getState().addToast(`Failed to sync from Dodo: ${err.message}`, 'error')
    } finally {
      setFetchingFromDodo(false)
    }
  }

  const handleProvisionAllRegionsToDodo = async () => {
    if (!window.confirm("This will connect to Dodo Payments and automatically create any missing products across all 8 regions (24 products total). Existing products will keep their IDs and update prices. Continue?")) {
      return
    }
    ...
    try {
      ...
      // Collect current regional prices from pricingTiers
      ...
      const createdCount = (data.results || []).filter((r: any) => r.method === 'CREATE').length
      const patchedCount = (data.results || []).filter((r: any) => r.method === 'PATCH').length
      useToastStore.getState().addToast(`🚀 Dodo Sync Complete: ${createdCount} created, ${patchedCount} updated!`, 'success')

    } catch (err: any) {
      useToastStore.getState().addToast(`Provisioning failed: ${err.message}`, 'error')
    } finally {
      setProvisioningAll(false)
    }
  }
  ...
  const handleSyncPricesToDodo = async () => {
    ...
    try {
      ...
      if (resp.ok && data.results) {
        ...
        // Automatically save any new or updated product IDs returned by Dodo
        if (data.updatedProductIds && Object.keys(data.updatedProductIds).length > 0) {
          ...
          setDodoProducts(newProductsMap)
          const productField = dodoTestMode ? "dodo_products_test" : "dodo_products_live"
          const updatePayload: any = { [productField]: newProductsMap }
          if (!dodoTestMode) updatePayload.dodo_products = newProductsMap
          await setDoc(doc(db, "settings", "global"), updatePayload, { merge: true })
        }
        ...
        if (allOk) {
          const createdCount = data.results.filter((r: any) => r.actionTaken === 'CREATED').length
          if (createdCount > 0) {
            useToastStore.getState().addToast(`✅ Synced to Dodo for ${regionCode} (${createdCount} products auto-created!)`, 'success')
          } else {
  ...
  return (
    <div className="space-y-8 w-full min-w-0 font-sans transition-all duration-300 t-text-primary">
          ...
          <div className="grid lg:grid-cols-3 gap-8 w-full min-w-0">
            <div className="lg:col-span-2 space-y-8 w-full min-w-0">
              ...
              <div className="p-4 sm:p-6 rounded-2xl border w-full min-w-0 t-card">
                ...
                {activeGateway === "dodo" && (
                  <div className="mt-6 p-4 rounded-xl border border-indigo-500/30 bg-indigo-500/5 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <RefreshCw className={`w-4 h-4 text-indigo-400 ${fetchingFromDodo || provisioningAll ? 'animate-spin' : ''}`} />
                        <span className="text-xs font-bold text-indigo-300">Automated Dodo Two-Way Catalog Sync</span>
                      </div>
                      <span className="text-[10px] px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 font-mono">
                        {dodoTestMode ? "TEST SANDBOX" : "LIVE PRODUCTION"}
                      </span>
                    </div>
                    <p className="text-[11px] text-zinc-400 leading-relaxed">
                      Zero hardcoding needed: When you change API keys, click <strong>Fetch & Sync</strong> to pull products & prices directly from Dodo into TakeoutFix, or <strong>Auto-Provision</strong> to create any missing products in Dodo with local currencies.
                    </p>
                    <div className="flex flex-wrap gap-2 pt-1">
                      <Button
                        type="button"
                        onClick={handleFetchProductsFromDodo}
                        disabled={fetchingFromDodo || provisioningAll}
                        className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs h-8 px-3 rounded-lg flex items-center gap-1.5"
                      >
                        {fetchingFromDodo ? <><RefreshCw className="w-3 h-3 animate-spin" /> Fetching from Dodo...</> : <>🔄 Fetch & Sync from Dodo</>}
                      </Button>
                      <Button
                        type="button"
                        onClick={handleProvisionAllRegionsToDodo}
                        disabled={fetchingFromDodo || provisioningAll}
                        className="bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-200 text-xs h-8 px-3 rounded-lg border border-zinc-700 flex items-center gap-1.5"
                      >
                        {provisioningAll ? <><RefreshCw className="w-3 h-3 animate-spin" /> Provisioning 8 Regions...</> : <>🚀 Auto-Provision All 8 Regions</>}
                      </Button>
                    </div>
                  </div>
                )}
              ...
              <div className="pt-6 border-t flex items-center justify-between flex-wrap gap-4 t-border">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    type="button"
                    onClick={handleFetchProductsFromDodo}
                    disabled={fetchingFromDodo || provisioningAll || syncingPrices}
                    className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white px-3 h-9 text-xs font-semibold rounded-lg flex items-center gap-1.5"
                  >
                    {fetchingFromDodo ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Fetching...</> : <>📥 Fetch from Dodo</>}
                  </Button>
                  <Button
                    type="button"
                    onClick={handleProvisionAllRegionsToDodo}
                    disabled={fetchingFromDodo || provisioningAll || syncingPrices}
                    className="bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-200 border border-zinc-700 px-3 h-9 text-xs font-semibold rounded-lg flex items-center gap-1.5"
                  >
                    {provisioningAll ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Provisioning...</> : <>🚀 Auto-Provision (8 Regions)</>}
                  </Button>
                  <Button
                    type="button"
                    onClick={handleSyncPricesToDodo}
                    disabled={syncingPrices || savingPricing || fetchingFromDodo}
                  ...
                  <Button 
                    ...
                    className="bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white px-5 h-9 text-xs font-semibold rounded-lg"
                  >
      ...
      {/* Dodo Fetched Products Catalog Modal */}
      {fetchedCatalogData && fetchedCatalogData.open && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl max-w-2xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
                ...
                <div>
                  <h3 className="text-sm font-bold text-white flex items-center gap-2">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-mono">
                      {fetchedCatalogData.count} Products Discovered
                    </span>
                  </h3>
                  <p className="text-[11px] text-zinc-400">
                    Fetched directly from Dodo Payments ({fetchedCatalogData.envMode.toUpperCase()} mode) and linked to TakeoutFix.
                  </p>
                </div>
              </div>
              <button
                onClick={() => setFetchedCatalogData(null)}
                className="w-7 h-7 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 flex items-center justify-center transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {/* Product Items List */}
            <div className="p-5 overflow-y-auto space-y-3 flex-1 divide-y divide-zinc-850">
              {fetchedCatalogData.products.length === 0 ? (
                <div className="text-center py-8 text-zinc-500 text-xs">
                  No products found in this Dodo Payments account.
                </div>
              ) : (
                fetchedCatalogData.products.map((prod: any, idx: number) => {
                  const pId = prod.product_id || prod.id || '';
                  const name = prod.name || 'Untitled Product';
                  let priceDisplay = '';
                  if (prod.price && typeof prod.price === 'object') {
                    const amt = (prod.price.price || prod.price.amount || 0) / 100;
                    const curr = prod.price.currency || 'USD';
                    priceDisplay = `${curr} ${amt.toFixed(2)}`;
                  } else if (typeof prod.price === 'number') {
                    priceDisplay = `${prod.currency || 'USD'} ${(prod.price / 100).toFixed(2)}`;
                  }

                  // Find which region and tier this product ID was mapped to
                  let mappedLocation = '';
                  if (fetchedCatalogData.mappedProducts) {
                    for (const [rCode, plans] of Object.entries(fetchedCatalogData.mappedProducts)) {
                      for (const [planCode, mappedId] of Object.entries(plans)) {
                        if (mappedId === pId) {
                          const regionObj = DODO_REGIONS.find(r => r.key === rCode);
                          const regionName = regionObj ? regionObj.label : rCode.toUpperCase();
                          const planLabel = PLAN_LABELS[planCode] || planCode;
                          mappedLocation = `${regionName} → ${planLabel}`;
                          break;
                        }
                      }
                      if (mappedLocation) break;
                    }
                  }

                  return (
                    <div key={pId || idx} className="pt-3 first:pt-0 flex items-center justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="text-xs font-semibold text-zinc-200 truncate flex items-center gap-2">
                          <span>{name}</span>
                          {mappedLocation && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded bg-indigo-500/15 text-indigo-300 font-normal border border-indigo-500/20">
                              {mappedLocation}
                            </span>
                          )}
                        </div>
                          ...
                          {pId && (
                            <button
                              ...
                              onClick={() => {
                                ...
                                useToastStore.getState().addToast('Copied Product ID!', 'info');
                              }}
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <span className="text-[9px] text-zinc-500">Synced ✓</span>
                      </div>
                    </div>
                  );
                })
            ...
            <div className="p-4 border-t border-zinc-800 bg-zinc-900/30 flex items-center justify-between">
              <span className="text-[11px] text-zinc-500">
                All discovered product IDs have been saved to Firestore.
              </span>
              <Button
                onClick={() => setFetchedCatalogData(null)}
                className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs h-8 px-4 rounded-lg font-semibold"
              >
                Done
              </Button>
            </div>
